<#
    VRR CREATOR — build a release package  (MAINTAINER SIDE, not for creators)
    ---------------------------------------------------------------------------
    Produces the two files VRR Creator needs on a GitHub release:
    ...
    VRRealms/Plugins/VRRealmsCreatorKit/ because VRR Creator refuses to extract
...
param(
    ...
    # 1.2 GB, so a bump tells VRR Creator to send them for a full re-download instead
...
if ($version -eq '1.0') {
    throw "VersionName is still the '1.0' placeholder. Set the real version in the .uplugin first — VRR Creator compares against it."
}
...
Write-Host ""
Write-Host "  VRR Creator shows the release notes to creators, so write them for creators." -ForegroundColor DarkGray
