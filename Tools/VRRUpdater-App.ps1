...
function Install-Release($entry) {
    ...
    try {
        ...
        # Verify straight after the install (CrazySketch 2026-09-21: "things need verification after update").
        # A patch only rewrites the plugin folder; the release's file table covers the whole kit, so this is
        # what catches a release whose sample content moved on, or a file the patch left behind. Invoke-Verify
        # shows the list and, when anything differs, the Repair button with its own explanation. When all is
        # well, say both things in one line so the install message is not lost behind the verify one.
        try {
            Invoke-Verify
            if ($script:VerifyManifest -and $script:VerifyBad.Count -eq 0) {
                $n = @($script:VerifyManifest.files.PSObject.Properties).Count
                Set-Status "Installed $now and verified: all $n shipped files match. Previous tools are in _VRRUpdaterBackup_$local. Reopen your project to use them." '#FF34D399'
            }
        } catch {
            Set-Status "Installed $now, but the file check could not run ($($_.Exception.Message)). Press Verify files to try again." '#FFEAB308'
        }
    }
    catch {
...
function Invoke-Verify {
    ...
    if ($bad.Count -eq 0) {
        ...
        Set-Status "$($bad.Count) of $($result.Count) shipped files differ ($mod modified, $mis missing). If you changed them on purpose, ignore this. Otherwise press Repair to restore the kit's copies from release $local - yours are backed up first. Your maps and avatars are never touched." '#FFEAB308'
    }
}
